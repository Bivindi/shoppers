        ...
        <div class="container">
            ...
            <div class="row">
                @if (count($subcategories) == 0)
                    <h1>Sub Category not available.</h1>
                @endif
               @foreach($subcategories as $category)
              <div class="col s12 m6 l3" >
               <div class="card">
                      <div class="card-image">
                        <a href="{{ url('admin/add-products',[$category->slug]) }}">
                        <img src="{{ asset('subcategory/'.$category->thumb_img) }}"
                            alt="{{ $category->thumb_img }}" height="200px">
                        </a>
                      </div>
                      <div class="card-action">
                       <a href="{{ url('admin/add-products',[$category->slug]) }}">{{ substr($category->name, 0, 20) }}</a>
