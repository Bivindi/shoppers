                ...
                <div class="row">
                    <div class="col s12 m12 l12">
                        ...
                        <ol class="breadcrumbs">
                            ...
                            <li class="active">Add Seller Policy</li>
                        </ol>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col s12 m12 l12">
                    <div class="card-panel">
                        <h4 class="header2">Add Seller Policy</h4>
                        <div class="row">
                            <form id="formValidate" class="col s12" action="{{ route('post:add_seller_policy') }}"
                                  method="post" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                @if(count($seller)>0)
                                    <input type="hidden" name="SellerPolicyId" value="{{ $seller->id }}">
                                    <input type="hidden" name="oldAboutImg" value="{{ $seller->image }}">
                                @endif
                                <div class="row">
                                    <div class="input-field col s12">
                                        <textarea id="summernote" name="desc" rows="10"
                                                  cols="10">{!! (count($seller)>0) ? $seller->desc : old('desc') !!}</textarea>
                                        <span class="error">{{ $errors->first('desc') }}</span>
                                    </div>
                                    <div class="col s12">
                                        <p>Cancellation Image</p>
                                        @if(count($seller)>0)
                                            <input type="file" id="input-file-events" name="seller_image"
                                                   class="dropify-event"
                                                   data-default-file="{{ asset('otherpages/'.$seller->image) }}"/>
                                        @else
                                            <input type="file" name="seller_image" class="dropify"
                                                   data-default-file=""/>
                                        @endif
                                    </div>
                                </div>
