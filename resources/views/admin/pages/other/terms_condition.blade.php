            ...
            <div class="row">
                <div class="col s12 m12 l12">
                    <div class="card-panel">
                        <h4 class="header2">Add Terms & Condition</h4>
                        <div class="row">
                            <form id="formValidate" class="col s12" action="{{ route('post:add_terms_condition') }}"
                                  method="post" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                @if(count($terms)>0)
                                    <input type="hidden" name="termsId" value="{{ $terms->id }}">
                                    <input type="hidden" name="oldAboutImg" value="{{ $terms->image }}">
                                @endif
                                <div class="row">
                                    <div class="input-field col s12">
                                        <textarea id="summernote" name="desc" rows="10"
                                                  cols="10">{!! (count($terms)>0) ? $terms->desc : old('desc') !!}</textarea>
                                        <span class="error">{{ $errors->first('desc') }}</span>
                                    </div>
                                    <div class="col s12">
                                        <p>Terms Image</p>
                                        @if(count($terms)>0)
                                            <input type="file" id="input-file-events" name="terms_image"
                                                   class="dropify-event"
                                                   data-default-file="{{ asset('otherpages/'.$terms->image) }}"/>
                                        @else
                                            <input type="file" name="terms_image" class="dropify"
                                                   data-default-file=""/>
                                        @endif
                                    </div>
                                </div>
