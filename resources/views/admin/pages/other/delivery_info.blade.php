                ...
                <div class="row">
                    <div class="col s12 m12 l12">
                        ...
                        <ol class="breadcrumbs">
                            ...
                            <li class="active">Add Delivery Info</li>
                        </ol>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col s12 m12 l12">
                    <div class="card-panel">
                        <h4 class="header2">Add Delivery Info</h4>
                        <div class="row">
                            <form id="formValidate" class="col s12" action="{{ route('post:add_delivery_info') }}"
                                  method="post" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                @if(count($delivery)>0)
                                    <input type="hidden" name="deliveryId" value="{{ $delivery->id }}">
                                    <input type="hidden" name="oldAboutImg" value="{{ $delivery->image }}">
                                @endif
                                <div class="row">
                                    <div class="input-field col s12">
                                        <textarea id="summernote" name="desc" rows="10"
                                                  cols="10">{!! (count($delivery)>0) ? $delivery->desc : old('desc') !!}</textarea>
                                        <span class="error">{{ $errors->first('desc') }}</span>
                                    </div>
                                    <div class="col s12">
                                        <p>Terms Image</p>
                                        @if(count($delivery)>0)
                                            <input type="file" id="input-file-events" name="delivery_image"
                                                   class="dropify-event"
                                                   data-default-file="{{ asset('otherpages/'.$delivery->image) }}"/>
                                        @else
                                            <input type="file" name="delivery_image" class="dropify"
                                                   data-default-file=""/>
                                        @endif
                                    </div>
                                </div>
