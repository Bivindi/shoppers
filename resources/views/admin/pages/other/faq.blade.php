            ...
            <div class="row">
                <div class="col s12 m12 l12">
                    <div class="card-panel">
                        <h4 class="header2">Add FAQ</h4>
                        <div class="row">
                            <form id="formValidate" class="col s12" action="{{ route('post:add_faq') }}"
                                  method="post" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                @if(count($faq)>0)
                                    <input type="hidden" name="faqId" value="{{ $faq->id }}">
                                    <input type="hidden" name="oldAboutImg" value="{{ $faq->image }}">
                                @endif
                                <div class="row">
                                    <div class="input-field col s12">
                                        <textarea id="summernote" name="desc" rows="10"
                                                  cols="10">{!! (count($faq)>0) ? $faq->desc : old('desc') !!}</textarea>
                                        <span class="error">{{ $errors->first('desc') }}</span>
                                    </div>
                                    <div class="col s12">
                                        <p>Terms Image</p>
                                        @if(count($faq)>0)
                                            <input type="file" id="input-file-events" name="faq_image"
                                                   class="dropify-event"
                                                   data-default-file="{{ asset('otherpages/'.$faq->image) }}"/>
                                        @else
                                            <input type="file" name="faq_image" class="dropify"
                                                   data-default-file=""/>
                                        @endif
                                    </div>
                                </div>
