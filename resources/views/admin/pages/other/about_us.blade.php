            ...
            <div class="row">
                <div class="col s12 m12 l12">
                    <div class="card-panel">
                        <h4 class="header2">Add About Us</h4>
                        <div class="row">
                            <form id="formValidate" class="col s12" action="{{ route('post:add_about_us') }}"
                                  method="post" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                @if(count($about)>0)
                                    <input type="hidden" name="AboutId" value="{{ $about->id }}">
                                    <input type="hidden" name="oldAboutImg" value="{{ $about->image }}">
                                @endif
                                <div class="row">
                                    <div class="input-field col s12">
                                        <textarea id="summernote" name="desc" rows="10"
                                                  cols="10">{!! (count($about)>0) ? $about->desc : old('desc') !!}</textarea>
                                        <span class="error">{{ $errors->first('desc') }}</span>
                                    </div>
                                    <div class="col s12">
                                        <p>About Image</p>
                                        @if(count($about)>0)
                                            <input type="file" id="input-file-events" name="about_image"
                                                   class="dropify-event"
                                                   data-default-file="{{ asset('otherpages/'.$about->image) }}"/>
                                        @else
                                            <input type="file" name="about_image" class="dropify"
                                                   data-default-file=""/>
                                        @endif
                                    </div>
                                </div>
