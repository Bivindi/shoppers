            ...
            <div class="container">
                <div class="row">
                    <div class="col s12 m12 l12">
                        <h5 class="breadcrumbs-title">Manage TaxClass</h5>
                        <ol class="breadcrumbs">
                            <li><a href="javascript:void(0);">TaxClass</a>
                            </li>
                            <li class="active">Manage TaxClass</li>
                        </ol>
                    </div>
                </div>
            </div>
                    ...
                    <div class="table-responsive">
                        <table id="example" class="display responsive nowrap" cellspacing="0" width="100%">
                            ...
                            @foreach($taxClasses as $key => $taxClass)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td class="name">{{ ucwords(strtolower($taxClass->name)) }}</td>
                                    <td class="name">{{ ucwords(strtolower($taxClass->tax_rate)) }}</td>
                                    <td class="name">{{ ucwords(strtolower($taxClass->type)) }}</td>
                                    <td>
                                        <div class="btn-group">
                                            <a href="{{ route('get:edit_tax_class', $taxClass->slug) }}"
                                            ...
                                            <button class="btn tooltipped btn-warning deleteMember"
                                                    data-delay="20" slug="{{ $taxClass->slug }}"
        ...
        $(document).on('click', '.deleteMember', function (e) {
            ...
            swal({
                ...
                if (isConfirm) {
                    $.ajax({
                        type: 'get',
                        url: '{{ route('get:delete_tax_class') }}',
                        data: {slug: slug},
